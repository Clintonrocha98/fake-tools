...
final readonly class ResolveDictKey
{
    public function handle(string $pixKey): DictEntry
    {
        ...
        if (!$entry instanceof DictEntry) {
            StarkbankLog::warning('fake-starkbank.dict: chave não registrada — 404 em vez de resolução vazia, porque é o throw do consumidor que segura o Payout em Withheld antes de transferir para um beneficiário em branco', [
                'pix_key' => $pixKey,
            ]);
        ...
        StarkbankLog::info('fake-starkbank.dict: chave resolvida — os blobs de agência e conta saem opacos de propósito, para que o consumidor os ecoe verbatim no POST /v2/transfer', [
