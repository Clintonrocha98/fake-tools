...
final readonly class OpenFiatDeposit
{
    ...
    {
        if (!config()->boolean('venue-fiat.deposit_enabled', default: true)) {
    ...
    {
        $supportedCurrency = config()->string('venue-fiat.supported_currency', 'BRL');
        $supportedMethod = config()->string('venue-fiat.supported_payment_method', 'Pix');
    ...
    private function guardDepositLimit(string $amount): void
    {
        ...
        $limit = config('venue-fiat.deposit_limit');
