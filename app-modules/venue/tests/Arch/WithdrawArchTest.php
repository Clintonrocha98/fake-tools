...
|--------------------------------------------------------------------------
|
| Scoped a `He4rt\Venue\Withdraw\Models` e `He4rt\Venue\Withdraw\Enums`, não ao
| namespace `He4rt\Venue\Withdraw` inteiro: um `toExtend`/`toImplement` module-wide
| falharia nas Actions/DTOs/Controllers do mesmo sub-domínio.
|
*/

arch('withdraw models extend the shared BaseModel')
    ->expect('He4rt\Venue\Withdraw\Models')
    ->toExtend(BaseModel::class);

arch('withdraw enums implement the Filament enum contracts')
    ->expect('He4rt\Venue\Withdraw\Enums')
