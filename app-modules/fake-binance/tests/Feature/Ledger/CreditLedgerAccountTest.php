...
it('creates the ledger account when crediting an asset for the first time', function (): void {
    $account = (new CreditLedgerAccount)->handle('BRL', '100000');
...
it('adds to the existing free balance on a second credit', function (): void {
    ...
    $credit->handle('USDC', '100');

    $account = $credit->handle('USDC', '50.5');
...
it('never touches the locked balance', function (): void {
    $account = (new CreditLedgerAccount)->handle('BRL', '10');
...
it('recovers when two concurrent creates race for the same new asset', function (): void {
    ...
    DB::listen(function ($query) use (&$raced): void {
        $isFirstLookup = !$raced
            ...
            && str_contains((string) $query->sql, 'fake_binance_ledger_accounts');

        if ($isFirstLookup) {
            $raced = true;

            DB::table('fake_binance_ledger_accounts')->insert([
    ...
    $account = (new CreditLedgerAccount)->handle('BRL', '100');
