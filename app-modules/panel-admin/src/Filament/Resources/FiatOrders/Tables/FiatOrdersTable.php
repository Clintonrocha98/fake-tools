...
class FiatOrdersTable
{
        ...
        return $table
            ->columns([
                TextColumn::make('order_no')
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.order_no'))
                    ->searchable()
                    ->copyable(),
                ...
                TextColumn::make('status')
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.status_lazy'))
                    ->badge(),
                TextColumn::make('forced_status')
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.override'))
                    ->badge()
                    ->placeholder('—'),
                TextColumn::make('forced_wire_status')
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.unknown_wire'))
                    ->placeholder('—')
                    ->color('danger'),
                IconColumn::make('frozen')
                    ->boolean()
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.frozen')),
                TextColumn::make('brcode_delay_reads')
                    ->label(__('panel-admin::fake-binance.fiat_orders.columns.brcode_delay'))
                    ->placeholder('—')
                    ->description(fn (FiatOrder $record): string => __('panel-admin::fake-binance.fiat_orders.columns.brcode_delay_description', ['count' => $record->brcode_reads_count])),
    ...
    private static function creditNowAction(): Action
    {
        return Action::make('creditNow')
            ->label(__('panel-admin::fake-binance.fiat_orders.actions.credit_now'))
            ->icon(Heroicon::OutlinedBolt)
            ->color('success')
            ->requiresConfirmation()
            ->modalDescription(__('panel-admin::fake-binance.fiat_orders.actions.credit_now_description'))
            ->action(function (FiatOrder $record): void {
                resolve(CreditFiatOrderNow::class)->handle($record);

                Notification::make()->title(__('panel-admin::fake-binance.fiat_orders.actions.credit_now_notification'))->success()->send();
            });
    }
    ...
    private static function forceStatusAction(): Action
    {
        return Action::make('forceStatus')
            ->label(__('panel-admin::fake-binance.fiat_orders.actions.force_status'))
            ->icon(Heroicon::OutlinedExclamationTriangle)
            ->color('danger')
            ->schema([
                Select::make('status')
                    ->label(__('panel-admin::fake-binance.fiat_orders.actions.status_field'))
            ...
            ->action(function (array $data, FiatOrder $record): void {
                ...
                Notification::make()->title(__('panel-admin::fake-binance.fiat_orders.actions.force_status_notification'))->success()->send();
            });
    }
    ...
    private static function emitUnknownAction(): Action
    {
        return Action::make('emitUnknown')
            ->label(__('panel-admin::fake-binance.fiat_orders.actions.emit_unknown'))
            ->icon(Heroicon::OutlinedQuestionMarkCircle)
            ->color('warning')
            ->schema([
                TextInput::make('wireStatus')
                    ->label(__('panel-admin::fake-binance.fiat_orders.actions.wire_status_field'))
                    ->required(),
            ])
            ->action(function (array $data, FiatOrder $record): void {
                ...
                Notification::make()->title(__('panel-admin::fake-binance.fiat_orders.actions.emit_unknown_notification'))->success()->send();
            });
    }
    ...
    private static function delayBrcodeAction(): Action
    {
        return Action::make('delayBrcode')
            ->label(__('panel-admin::fake-binance.fiat_orders.actions.delay_brcode'))
            ->icon(Heroicon::OutlinedClock)
            ->color('gray')
            ->fillForm(fn (FiatOrder $record): array => ['reads' => $record->brcode_delay_reads])
            ->schema([
                TextInput::make('reads')
                    ->label(__('panel-admin::fake-binance.fiat_orders.actions.reads_field'))
                    ->numeric()
                    ->minValue(0)
                    ->helperText(__('panel-admin::fake-binance.fiat_orders.actions.reads_helper')),
            ])
            ->action(function (array $data, FiatOrder $record): void {
                ...
                Notification::make()->title(__('panel-admin::fake-binance.fiat_orders.actions.delay_brcode_notification'))->success()->send();
            });
    }
    ...
    private static function toggleFrozenAction(): Action
    {
        return Action::make('toggleFrozen')
            ->label(fn (FiatOrder $record): string => __('panel-admin::fake-binance.fiat_orders.actions.'.($record->frozen ? 'unfreeze' : 'freeze')))
            ...
            ->action(function (FiatOrder $record): void {
                ...
                Notification::make()->title(__('panel-admin::fake-binance.fiat_orders.actions.frozen_notification'))->success()->send();
            });
    }
}
