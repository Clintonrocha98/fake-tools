 ...
 *
 * Pest auto-namespaces our module tests as `Appmodules\<module>\tests\...`
 * (lowercase `tests`, derived from the `app-modules/` directory — not the PHP
 * `He4rt\` namespace), so they match nothing, get filtered OUT of every shard,
 * and run in NO shard — module tests would be silently skipped in sharded CI
 * while it stayed green. This plugin generalizes that one regex to `([^:]+)`
 * so every test class is discovered regardless of namespace.
 *
