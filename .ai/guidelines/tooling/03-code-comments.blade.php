...
## Comment prose is written in Brazilian Portuguese (pt_BR)

The comment exists for the reader, and this team reads and reviews in pt_BR. Write the
prose — line comments and docblock text alike — in Portuguese.

Never translated, in any comment:
...
- **Test names quoted as evidence.** `refuses a transfer of a different mint` is a
  pointer to a real test; translating it breaks the reference.
- **ADR titles and `CONTEXT.md` terms.** The glossary owns the vocabulary — see the
  domain-docs guideline. Use the term as defined, in the language it is defined in.
- **Domain and protocol vocabulary the project already speaks in English** — off-ramp,
  webhook, share token, intake, leg, mint. Forcing a translation here invents a synonym
  the glossary does not have, which is worse than the English word.

@verbatim
<code-snippet name="Same invariant, team's language" lang="php">
// BAD — the rule is right, the language is not the one the team reviews in:
...
// GOOD — pt_BR, with identifiers and domain vocabulary left alone:
...
Comments already written in English are **not** a translation campaign. They convert when
the rule that governs them is rewritten anyway — the replanning rule above already deletes
and replaces, and what it writes is born in pt_BR.
...
5. Every comment you wrote is in pt_BR, with identifiers, quoted test names and glossary
   terms left in their original form.
