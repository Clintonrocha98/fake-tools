...
- **Preencher parcial e expirar o resto** — executes only the given fraction and responds `EXPIRED`. Empty uses half.
- **Recusar com código** — responds with the `/api/v3` error envelope and creates no order at all.
- **Responder REJECTED** — HTTP 200 with `REJECTED` and a zeroed fill.
- **Emitir vocabulário desconhecido** — executes normally, but the wire responds with whatever status you type.
