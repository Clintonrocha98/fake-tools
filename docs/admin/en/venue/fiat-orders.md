...
A fiat order (`POST /sapi/v1/fiat/deposit`) is created by the consuming application to announce a PIX deposit. By default it credits the ledger on its own once it is older than `venue-fiat.advance_seconds` — no action needed for the happy path.
