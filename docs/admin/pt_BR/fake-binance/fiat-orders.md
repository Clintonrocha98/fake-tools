...
Uma fiat order (`POST /sapi/v1/fiat/deposit`) é criada pela aplicação consumidora para anunciar um depósito PIX. Por padrão ela credita o ledger sozinha assim que fica mais velha que `fake-binance-fiat.advance_seconds` — nenhuma ação é necessária para o happy path.
